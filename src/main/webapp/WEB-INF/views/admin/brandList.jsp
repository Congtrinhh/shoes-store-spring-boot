...
<link rel="stylesheet" href="${base}/css/admin/brandList/style.css">
<script defer src="${base}/js/common/utils.js"></script>
<script defer src="${base}/js/admin/lib/adminSidebar.js"></script>
<script defer src="${base}/js/admin/brandList/main.js"></script>
...
<body>
	...
	<div id="app">
		...
		<main class="content px-3">
			<h2 class="page-header ">Hãng</h2>
			<div class="new">
				<a href="${base }/admin/brand/create" class="btn"><i
					class="far fa-plus-square"></i> create new</a>
			</div>
			<div class="search-wrapper">
				<h3 class="filter-header ">Bộ lọc</h3>
				<form:form action="${base }/admin/brand/list" method="get"
			...
			<div class="total-item-of-query">
				<span>[${brandsWithPaging.totalItems}</span> <span>bản ghi]</span>
			</div>
			<table id="product" class="">
				...
				<tbody>
					<c:forEach items="${brandsWithPaging.data}" var="b">
						<tr class="product-row">
							<td>${b.name}</td>
							<td>${b.description}</td>
							<td <c:if test="${b.status==true}">class="status on"</c:if>
								<c:if test="${b.status==false}">class="status"</c:if>>
							</td>
							<td>${b.createdDate }</td>
							<td>${b.updatedDate }</td>
							<td>
								<!-- for update --> <a class="btn btn-update"
								href="${base}/admin/brand/update/${b.id}" item-id="${b.id}"><i
									class="fas fa-pen-square"></i></a> <!-- for delete - button trigger modal -->
								<button type="button" item-id="${b.id}"
	...
	<script type="text/javascript">		
		$( document ).ready(function() {
				$("#paging").pagination({
					currentPage: ${brandsWithPaging.currentPage},
			        items: ${brandsWithPaging.totalItems},
