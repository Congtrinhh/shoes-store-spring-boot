...
<link rel="stylesheet"
	href="${base}/css/admin/productSizeList/style.css">
<script defer src="${base}/js/common/utils.js"></script>
<script defer src="${base}/js/admin/lib/adminSidebar.js"></script>
<script defer src="${base}/js/admin/productSizeList/main.js"></script>
...
<body>
	...
	<div id="app">
		...
		<main class="content px-3">
			...
			<!-- new item -->
			<div class="new">
				<a href="${base }/admin/product-size/create" class="btn"><i
					class="far fa-plus-square"></i> create new</a>
			</div>

			<div class="search-wrapper">
				<h3 class="filter-header ">Bộ lọc</h3>
				<form:form action="${base }/admin/product-size/list" method="get"
					...
					<div class="select-wrapper">
						...
						<form:select path="sizeId" class="form-select">
							<form:option value="">All</form:option>
							<form:options items="${sizes }" itemLabel="sizeNo" itemValue="id"/>
						</form:select>
					</div>
			...
			<div class="total-item-of-query">
				<span>[${productSizesWithPaging.totalItems}</span> <span>bản
					ghi]</span>
			</div>
			...
			<table id="product" class="">
				...
				<tbody>
					<c:forEach items="${productSizesWithPaging.data}" var="ps">
						<tr class="product-row">
							<td><a href="${base }/product-detail/${ps.product.seo}"
								style="text-decoration: underline;">${ps.product.name }</a></td>
							<td <c:if test="${ps.status==true}">class="status on"</c:if>
								<c:if test="${ps.status==false}">class="status"</c:if>></td>
							<td>${ps.quantity }</td>
							<td>${ps.discountPercent }%</td>								
							<td>${ps.createdDate }</td>
							<td>${ps.updatedDate }</td>
							<td>
								<!-- for update --> <a class="btn btn-update"
								href="${base}/admin/product-size/update/${ps.id}"
								item-id="${ps.id}"><i class="fas fa-pen-square"></i></a> <!-- for delete - button trigger modal -->
								<button type="button" item-id="${ps.id}"
	...
	<script type="text/javascript">		
		$( document ).ready(function() {
				$("#paging").pagination({
					currentPage: ${productSizesWithPaging.currentPage},
			        items: ${productSizesWithPaging.totalItems},
