...
<link rel="stylesheet" href="${base}/css/admin/sizeList/style.css">
<script defer src="${base}/js/common/utils.js"></script>
<script defer src="${base}/js/admin/lib/adminSidebar.js"></script>
<script defer src="${base}/js/admin/sizeList/main.js"></script>
...
<body>
	...
	<div id="app">
		...
		<h2 class="page-header ">Size</h2>
			
			<div class="new">
				<a href="${base }/admin/size/create" class="btn"><i
					class="far fa-plus-square"></i> create new</a>
			</div>
			
			<div class="search-wrapper">
			<h3 class="filter-header ">Bộ lọc</h3>
				<form:form action="${base }/admin/size/list" method="get"
			...
			<div class="total-item-of-query">
				<span>[${sizesWithPaging.totalItems}</span> <span>bản
					ghi]</span>
			</div>
			...
			<table id="product" class="">
				...
				<tbody>
					<c:forEach items="${sizesWithPaging.data}" var="s">
						<tr class="product-row">
							<td>${s.sizeNo}</td>
							<td 
								<c:if test="${s.status==true}">class="status on"</c:if> 
								<c:if test="${s.status==false}">class="status"</c:if> 
							>
							</td>
							<td>${s.createdDate }</td>
							<td>${s.updatedDate }</td>
							...
							<!-- for update --> <a class="btn btn-update"
								href="${base}/admin/size/update/${s.id}" item-id="${s.id}"><i
									class="fas fa-pen-square"></i></a>
									
								 <!-- for delete - button trigger modal -->
								<button type="button" item-id="${s.id}"
	...
	<script type="text/javascript">		
		$( document ).ready(function() {
				$("#paging").pagination({
					currentPage: ${sizesWithPaging.currentPage},
			        items: ${sizesWithPaging.totalItems},
